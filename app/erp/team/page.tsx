...
const departmentColors: Record<string, string> = {
  executive: 'bg-purple-900/20 text-purple-700',
  'project management': 'bg-blue-900/20 text-blue-400',
  engineering: 'bg-emerald-900/20 text-green-700',
  consulting: 'bg-orange-100 text-orange-700',
  sales: 'bg-pink-100 text-pink-700',
};

const statusColors: Record<string, string> = {
  available: 'bg-emerald-900/20 text-green-700',
  busy: 'bg-amber-900/20 text-yellow-700',
  away: 'bg-slate-800 text-slate-300',
  offline: 'bg-red-900/20 text-red-700',
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">Manage and view your organization's team</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center border border-slate-800 rounded-lg overflow-hidden">
            <button
              ...
              className={cn(
                'px-3 py-2 text-sm',
                viewMode === 'grid' ? 'bg-blue-900/20 text-blue-400' : 'text-slate-400 hover:bg-slate-950/50'
              )}
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <Card key={user.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="pt-12 px-6 pb-6">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  <Badge className={departmentColors[user.department?.toLowerCase() || 'other'] || 'bg-slate-800 text-slate-300'}>
                    {user.department}
                  </Badge>
                  <Badge className={statusColors[user.status || 'offline'] || 'bg-slate-800 text-slate-300'}>
                    {user.status || 'offline'}
                  </Badge>
                </div>
      ...
      {showDetails && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            {/* Profile Header */}
            <div className="h-32 bg-linear-to-r from-blue-500 to-blue-600 relative">
              <button
                onClick={() => setShowDetails(false)}
                className="absolute top-4 right-4 text-white hover:bg-slate-900 hover:bg-opacity-20 rounded-full p-2"
              >
            ...
            <div className="relative px-6 pb-6">
              ...
              <div className="pt-20">
                <div className="mb-6">
                  <h1 className="text-3xl font-bold text-white">{selectedUser.fullName}</h1>
                  <p className="text-lg text-slate-400">{selectedUser.title}</p>
                </div>

                {/* Badges */}
                <div className="flex flex-wrap gap-2 mb-6">
                  <Badge className={departmentColors[selectedUser.department?.toLowerCase() || 'other'] || 'bg-slate-800 text-slate-300'}>
                    {selectedUser.department}
                  </Badge>
                  <Badge className={statusColors[selectedUser.status || 'offline'] || 'bg-slate-800 text-slate-300'}>
                    {selectedUser.status || 'offline'}
                  </Badge>
                  {selectedUser.isActive && (
                    <Badge className="bg-emerald-900/20 text-green-700">Active</Badge>
                  )}
                </div>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-slate-400">{selectedUser.bio}</p>
                </div>

                {/* Contact Information */}
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <h2 className="text-sm font-semibold text-white mb-2">Contact Information</h2>
                ...
                <div className="border-t border-slate-800 pt-6">
                  <h2 className="text-sm font-semibold text-white mb-2">Activity</h2>
                  <div className="space-y-2 text-sm text-slate-400">
                    <p>
              ...
              <form onSubmit={handleAddMember} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 border border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                    rows={3}
                  />
                </div>
                  ...
                  {formData.profileImage && (
                    <div className="mt-3 flex justify-center">
                      <div className="relative">
                        ...
                        />
                        <p className="text-xs text-slate-400 text-center mt-2">Preview</p>
                      </div>
                    </div>
                  )}
                </div>
