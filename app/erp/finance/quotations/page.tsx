    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Advanced quotation management system</p>
        </div>
        <div className="flex gap-2">
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            <button onClick={() => setViewMode('table')} className={cn('px-3 py-1.5 rounded text-sm font-medium', viewMode === 'table' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600')}>Table</button>
            <button onClick={() => setViewMode('grid')} className={cn('px-3 py-1.5 rounded text-sm font-medium', viewMode === 'grid' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600')}>Grid</button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-6"><p className="text-sm text-gray-500">Total Value</p><p className="text-2xl font-bold text-gray-900 mt-2">{formatCurrency(stats.totalValue)}</p><p className="text-xs text-gray-500 mt-1">{stats.totalQuotations} quotations</p></Card>
        <Card className="p-6"><p className="text-sm text-gray-500">Pending</p><p className="text-2xl font-bold text-yellow-600 mt-2">{formatCurrency(stats.sentValue)}</p><p className="text-xs text-gray-500 mt-1">{stats.pendingCount} awaiting</p></Card>
        <Card className="p-6"><p className="text-sm text-gray-500">Accepted</p><p className="text-2xl font-bold text-green-600 mt-2">{formatCurrency(stats.acceptedValue)}</p><p className="text-xs text-gray-500 mt-1">{stats.acceptedCount} conversions</p></Card>
        <Card className="p-6"><p className="text-sm text-gray-500">Conversion Rate</p><p className="text-2xl font-bold text-purple-600 mt-2">{stats.conversionRate}%</p><p className="text-xs text-gray-500 mt-1">Avg: {formatCurrency(stats.avgQuotationValue)}</p></Card>
      </div>
      ...
      <Card className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <select value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
          ...
          <select value={dateRangeFilter} onChange={(e) => setDateRangeFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
            <option value="all">All Dates</option>
            <option value="30days">Last 30 Days</option>
          </select>
          <div className="flex gap-2 overflow-x-auto">
            {['all', 'draft', 'sent', 'accepted', 'rejected'].map((status) => (<button key={status} onClick={() => setStatusFilter(status)} className={cn('px-3 py-2 text-sm font-medium rounded-lg whitespace-nowrap', statusFilter === status ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600')}>{status}</button>))}
          </div>
        </div>
      </Card>
              ...
              {filteredQuotations.map((quote) => {
                ...
                return (
                  <TableRow key={quote.id}>
                    <TableCell><p className="font-medium text-blue-600 cursor-pointer hover:underline" onClick={() => handleViewDetails(quote)}>{quote.quotationNumber}</p></TableCell>
                    <TableCell><p>{company?.name || '-'}</p></TableCell>
                    <TableCell><p className="text-gray-500">{quote.issueDate ? new Date(quote.issueDate).toLocaleDateString() : '-'}</p></TableCell>
                    <TableCell><p className="text-gray-500">{quote.validUntil ? new Date(quote.validUntil).toLocaleDateString() : '-'}</p></TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <button onClick={() => handleViewDetails(quote)} className="text-blue-600 hover:text-blue-800" title="View"><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                        <button onClick={() => handleOpenModal(quote)} className="text-gray-600 hover:text-gray-800" title="Edit"><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        {quote.status === 'draft' && <button onClick={() => handleStatusChange(quote.id, 'sent')} className="text-green-600 hover:text-green-800" title="Send"><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg></button>}
                      </div>
                    </TableCell>
                  </TableRow>
                );
            ...
            return (
              <Card key={quote.id} className="p-6 hover:shadow-lg transition-shadow cursor-pointer" onClick={() => handleViewDetails(quote)}>
                <div className="flex justify-between items-start mb-4">
                  <div><h3 className="font-semibold text-gray-900">{quote.quotationNumber}</h3><p className="text-sm text-gray-500">{quote.title}</p></div>
                  <Badge status={quote.status}>{quote.status}</Badge>
                </div>
                <div className="mb-3"><p className="text-sm text-gray-500">Client</p><p className="font-medium">{company?.name || '-'}</p></div>
                <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                  <div><p className="text-gray-500">Issue Date</p><p className="font-medium">{quote.issueDate ? new Date(quote.issueDate).toLocaleDateString() : '-'}</p></div>
                  <div><p className="text-gray-500">Valid Until</p><p className="font-medium">{quote.validUntil ? new Date(quote.validUntil).toLocaleDateString() : '-'}</p></div>
                </div>
                <div className="pt-4 border-t"><p className="text-2xl font-bold text-blue-600">{formatCurrency(quote.total)}</p></div>
              </Card>
            );
          })}
        </div>
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex gap-6 flex-1">
                  {currentQuotation.companyLogo || getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.logoUrl ? (
                    ...
                    <div className="bg-white p-3 rounded-lg shadow-sm flex items-center justify-center h-16 w-16">
                      <svg className="h-8 w-8 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m0 0h5" /></svg>
                    </div>
                  )}
                  <div className="flex-1">
                    <div>
                      <p className="text-sm text-blue-600 font-medium">Quotation</p>
                      <h1 className="text-3xl font-bold text-gray-900">{currentQuotation.quotationNumber}</h1>
                    </div>
                    <div className="mt-3 grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs text-gray-600">Company</p>
                        <p className="font-semibold text-gray-900">{currentQuotation.companyName || getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.name || 'N/A'}</p>
                      </div>
                      <div>
                ...
                <button onClick={() => setShowDetailModal(false)} className="text-gray-500 hover:text-gray-700 p-2">
            ...
            <div className="p-8 border-b border-gray-200">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                  ...
                  <p className="text-sm text-gray-900 font-medium mt-1">{currentQuotation.companyEmail || getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.email || '-'}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">Phone</p>
                  <p className="text-sm text-gray-900 font-medium mt-1">{currentQuotation.companyPhone || getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.phone || '-'}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">Website</p>
                  <p className="text-sm text-blue-600 font-medium mt-1">{currentQuotation.companyWebsite || getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.website || '-'}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">Industry</p>
                  <p className="text-sm text-gray-900 font-medium mt-1">{getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.industry || '-'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8">
              <div className="grid grid-cols-4 gap-4 mb-8 pb-8 border-b">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900 mt-1">{currentQuotation.issueDate ? new Date(currentQuotation.issueDate).toLocaleDateString() : '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 font-medium">Valid Until</p>
                  <p className="text-lg font-semibold text-gray-900 mt-1">{currentQuotation.validUntil ? new Date(currentQuotation.validUntil).toLocaleDateString() : '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 font-medium">Client Contact</p>
                  <p className="text-lg font-semibold text-gray-900 mt-1">{getClientCompany(currentQuotation.leadId || currentQuotation.clientId)?.name || '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 font-medium">Currency</p>
                  <p className="text-lg font-semibold text-gray-900 mt-1">{currentQuotation.currency}</p>
                </div>
              </div>
                    ...
                    <tbody>
                      {currentQuotation.items?.map((item: any) => (
                        <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
                          ...
                          <td className="text-right py-3 px-4 text-gray-600">{formatCurrency(item.unitPrice)}</td>
                          <td className="text-right py-3 px-4 font-semibold text-gray-900">{formatCurrency(item.total)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="bg-linear-to-br from-blue-50 to-blue-100 p-6 rounded-lg border border-blue-200">
                  <div className="space-y-3">
                    <div className="flex justify-between items-center pb-3 border-b border-blue-200">
                      ...
                      <p className="font-semibold text-gray-900">{formatCurrency(currentQuotation.subtotal)}</p>
                    </div>
                    <div className="flex justify-between items-center pb-3 border-b border-blue-200">
                      <p className="text-gray-700">Tax ({currentQuotation.taxRate * 100}%)</p>
                      <p className="font-semibold text-gray-900">{formatCurrency(currentQuotation.taxAmount)}</p>
                    </div>
                    {currentQuotation.discountAmount > 0 && (
                      <div className="flex justify-between items-center pb-3 border-b border-blue-200">
                        <p className="text-gray-700">Discount</p>
                        <p className="font-semibold text-gray-900">-{formatCurrency(currentQuotation.discountAmount)}</p>
                      </div>
                    )}
                    <div className="flex justify-between items-center pt-2">
                      <p className="text-lg font-bold text-gray-900">Total Amount</p>
                      <p className="text-3xl font-bold text-blue-600">{formatCurrency(currentQuotation.total)}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {currentQuotation.termsConditions && (
                <div className="mt-8 pt-8 border-t border-gray-200">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <p className="text-sm text-gray-700 whitespace-pre-wrap">{currentQuotation.termsConditions}</p>
                  </div>
                </div>
              )}
              ...
              {currentQuotation.companyAddress && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <p className="text-gray-600">{currentQuotation.companyAddress}</p>
                </div>
              )}
            </div>

            {/* Action Buttons */}
            <div className="p-8 bg-gray-50 border-t border-gray-200 flex gap-3 sticky bottom-0">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-5xl w-full max-h-[95vh] overflow-y-auto shadow-2xl">
            ...
            <div className="bg-linear-to-r from-blue-50 to-blue-100 px-8 py-6 border-b border-blue-200 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{currentQuotation ? '✎ Edit Quotation' : '+ Create New Quotation'}</h2>
                <p className="text-sm text-gray-600 mt-1">Manage your quotation details and pricing</p>
              </div>
              <button type="button" onClick={() => setShowModal(false)} className="text-gray-500 hover:text-gray-700 p-2 bg-white rounded-lg">
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="border-b pb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <select value={formData.clientId} onChange={(e) => setFormData({ ...formData, clientId: e.target.value })} required className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:outline-none bg-white hover:border-gray-400 transition">
                      <option value="">Choose a client...</option>
                      {mockClients.map(c => <option key={c.id} value={c.id}>{mockCompanies.find(co => co.id === c.companyId)?.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2">🏷️ Quotation # *</label>
              ...
              <div className="border-b pb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:outline-none bg-white">
              ...
              <div className="border-b pb-8">
                ...
                {/* Table Header */}
                <div className="mb-3">
                  <div className="grid grid-cols-12 gap-3 px-4 py-3 bg-gray-100 rounded-lg border border-gray-200">
                    <div className="col-span-6 text-xs font-bold text-gray-700">DESCRIPTION</div>
                    <div className="col-span-2 text-xs font-bold text-gray-700 text-right">QTY</div>
                    <div className="col-span-2 text-xs font-bold text-gray-700 text-right">RATE</div>
                    <div className="col-span-1 text-xs font-bold text-gray-700 text-right">ACTION</div>
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-4">
                  {formData.lineItems.map((item, idx) => (
                    <div key={item.id} className="grid grid-cols-12 gap-3 items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                ...
                {/* Add Item Button */}
                <Button type="button" size="sm" onClick={addLineItem} variant="outline" className="border-dashed border-2 w-full text-gray-600 hover:text-gray-900">
                  + Add Line Item
                </Button>
              </div>
              ...
              <div>
                ...
                <label className="block text-sm font-semibold text-gray-900 mb-2">Terms Details</label>
                <textarea 
                  rows={3} 
                  value={formData.termsConditions} 
                  onChange={(e) => setFormData({ ...formData, termsConditions: e.target.value })} 
                  placeholder="e.g., 50% upfront, 50% on completion. Net 30 payment terms."
                  className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:outline-none resize-none"
                />
              </div>
